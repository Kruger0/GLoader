...
// environment
uniform sampler2D	u_tex_environment;
...
void main() {
	...
	vec3 col_environment = texture2D(u_tex_environment, uv_oct).rgb;
	final_col.rgb = mix(final_col.rgb, mix(col_environment * final_col.rgb, final_col.rgb, rough_fac), metal_fac);
